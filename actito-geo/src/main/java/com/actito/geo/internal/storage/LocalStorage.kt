package com.actito.geo.internal.storage
...
internal class LocalStorage(context: Context) {
    ...
    var monitoredRegions: Map<String, ActitoRegion>
        get() {
            val jsonStr = sharedPreferences.getString(PREFERENCE_MONITORED_REGIONS, null)
                ?: return emptyMap()

            try {
                val type = Types.newParameterizedType(List::class.java, ActitoRegion::class.java)
                val adapter = Actito.moshi.adapter<List<ActitoRegion>>(type)
    ...
    var monitoredBeacons: List<ActitoBeacon>
        get() {
            val jsonStr = sharedPreferences.getString(PREFERENCE_MONITORED_BEACONS, null)
                ?: return emptyList()

            try {
                val type = Types.newParameterizedType(List::class.java, ActitoBeacon::class.java)
                val adapter = Actito.moshi.adapter<List<ActitoBeacon>>(type)
    ...
    fun updateRegionSessions(location: ActitoLocation) {
    ...
    var beaconSessions: Map<String, ActitoBeaconSession>
        get() {
            val jsonStr = sharedPreferences.getString(PREFERENCE_BEACON_SESSIONS, null)
                ?: return emptyMap()

            try {
                val type = Types.newParameterizedType(List::class.java, ActitoBeaconSession::class.java)
                val adapter = Actito.moshi.adapter<List<ActitoBeaconSession>>(type)
    ...
    fun addBeaconSession(session: ActitoBeaconSession) {
        beaconSessions = beaconSessions.toMutableMap().apply {
            put(session.regionId, session)
        }
    }

    fun updateBeaconSession(beacons: List<ActitoBeacon>, location: Location?) {
        if (beacons.isEmpty()) {
    ...
    fun removeBeaconSession(session: ActitoBeaconSession) {
        beaconSessions = beaconSessions.toMutableMap().apply {
            remove(session.regionId)
        }
    }

...
internal class ActitoSharedPreferences(context: Context) {
    ...
    var transport: ActitoTransport?
        get() {
            return sharedPreferences.getString(PREFERENCE_TRANSPORT, null)?.let {
                try {
                    Actito.moshi.adapter(ActitoTransport::class.java).fromJson(it)
                } catch (e: Exception) {
    ...
    var subscription: ActitoPushSubscription?
        get() {
            return sharedPreferences.getString(PREFERENCE_SUBSCRIPTION, null)?.let {
                try {
                    Actito.moshi.adapter(ActitoPushSubscription::class.java).fromJson(it)
                } catch (e: Exception) {

package com.actito.sample.user.inbox.core
...
internal abstract class BaseViewModel : ViewModel() {
    ...
    private val eventChannel = Channel<com.actito.sample.user.inbox.ktx.Event>(Channel.BUFFERED)
    internal val eventsFlow = eventChannel.receiveAsFlow()

    internal fun showSnackBar(message: String) {
        viewModelScope.launch {
            eventChannel.send(com.actito.sample.user.inbox.ktx.Event.ShowSnackBar(message))
        }
    }
